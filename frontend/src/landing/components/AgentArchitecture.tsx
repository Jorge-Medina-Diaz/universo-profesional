      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-20">
          <h2 className="font-display text-ink text-4xl md:text-6xl leading-[1.05] tracking-tight mb-6">
            Arquitectura que
            <br />
            <span className="text-stone">piensa contigo.</span>
          </h2>
        ...
        <div className="grid md:grid-cols-5 gap-4 mb-20">
          {STEPS.map((step, i) => (
            ...
            >
              <div className="rounded-xl border border-gray-200 bg-gray-50 p-5 hover:border-gray-300 transition-colors duration-300">
                <div className="w-9 h-9 rounded-lg bg-gray-100 border border-gray-200 flex items-center justify-center text-stone mb-3">
                  <step.icon size={16} />
                </div>
                <h4 className="text-sm font-medium text-ink mb-1.5">{step.title}</h4>
                <p className="text-[11px] text-stone leading-relaxed">{step.desc}</p>
              </div>
              {i < STEPS.length - 1 && (
                <div className="hidden md:flex absolute -right-2 top-1/2 -translate-y-1/2 z-10">
                  <motion.div animate={{ x: [0, 4, 0] }} transition={{ repeat: Infinity, duration: 1.5, delay: i * 0.2 }}>
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                      <path d="M4 2l6 5-6 5" stroke="rgba(0,0,0,0.12)" strokeWidth="1.5" strokeLinecap="round" />
                    </svg>
                  </motion.div>
                </div>
              )}
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          ...
          >
            <h3 className="font-display text-xl text-ink mb-2">4 capas de memoria</h3>
            <p className="text-sm text-stone mb-6">El agente mejora contigo sin fine-tuning.</p>
            <div className="space-y-3">
              {MEMORY.map((mem, i) => (
                ...
                >
                  ...
                  <div>
                    <div className="text-xs font-medium text-ink">{mem.label}</div>
                    <div className="text-[11px] text-stone">{mem.desc}</div>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <motion.div
            ...
            transition={{ duration: 0.5 }}
          >
            <h3 className="font-display text-xl text-ink mb-2">Todo tipo de entidad</h3>
            <p className="text-sm text-stone mb-6">Cada faceta de tu trayectoria tiene su lugar.</p>
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <motion.div
                  key={ent.label}
                  className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gray-100 border border-gray-200"
                ...
                >
                  ...
                  <span className="text-xs text-stone">{ent.label}</span>
                </motion.div>
              ))}
            </div>
          </motion.div>
