...
export function UniverseNutritionDemo() {
  ...
  return (
    <section className="py-32 md:py-40 bg-canvas overflow-hidden">
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="font-display text-ink text-4xl md:text-6xl leading-[1.05] tracking-tight mb-6">
            Tu perfil se
            <br />
            <span className="text-stone">construye solo.</span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 gap-10 max-w-4xl mx-auto">
                  ...
                  <div
                    className={`w-9 h-9 rounded-lg flex items-center justify-center transition-colors duration-500 ${
                      active
                        ? "bg-ink text-canvas"
                        : done
                        ? "bg-[#6ece9d]/20 text-[#4a9e6f]"
                        : "bg-gray-100 text-stone"
                    }`}
                  >
                    {done ? <CheckCircle2 size={16} /> : <s.icon size={16} />}
                  </div>
                  <span className={`text-sm font-medium transition-colors ${active ? "text-ink" : done ? "text-stone" : "text-gray-400"}`}>
                    {s.label}
                  </span>
                  {active && (
                    <motion.div
          ...
          {/* Visual stage */}
          <div className="relative min-h-[300px] rounded-2xl border border-gray-200 bg-gray-50 overflow-hidden p-5">
            <AnimatePresence mode="wait">
              {stepId === "upload" && (
                <motion.div key="upload" className="absolute inset-0 flex flex-col items-center justify-center p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <span className="text-sm font-medium text-stone">LinkedIn_Export.zip</span>
                  <span className="text-xs text-gray-400 mt-1">2.4 MB</span>
                </motion.div>
              )}

              {stepId === "parse" && (
                <motion.div key="parse" className="absolute inset-0 p-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <div className="text-[11px] text-stone mb-3 uppercase tracking-wider">Entidades detectadas</div>
                  <div className="space-y-2">
                    {prof.items.map((item, i) => (
                      <motion.div key={item.label} className="flex items-center gap-2.5 p-2.5 rounded-lg bg-gray-100 border border-gray-200" initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}>
              ...
              {stepId === "match" && (
                <motion.div key="match" className="absolute inset-0 p-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <div className="text-[11px] text-stone mb-3 uppercase tracking-wider">Relaciones descubiertas</div>
                  <div className="space-y-3">
                    {[
                      ...
                      <motion.div key={i} className="flex items-center gap-2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.2 }}>
                        <span className="text-xs text-stone">{pair.a}</span>
                        <div className="flex-1 h-px bg-gray-300 relative">
                          <motion.div className="absolute inset-y-0 left-0 bg-[#6ece9d]/40" initial={{ width: 0 }} animate={{ width: "100%" }} transition={{ delay: 0.3 + i * 0.2, duration: 0.6 }} />
                        </div>
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#6ece9d]/15 text-[#4a9e6f] font-medium">{pair.rel}</span>
                        <div className="flex-1 h-px bg-gray-200" />
                        <span className="text-xs text-stone">{pair.b}</span>
                      </motion.div>
                    ))}
                  </div>
                </motion.div>
              ...
              {stepId === "grow" && (
                <motion.div key="grow" className="absolute inset-0 p-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <div className="text-[11px] text-stone mb-3 uppercase tracking-wider">Tu universo ahora</div>
                  <div className="relative h-full">
                    <svg className="absolute inset-0 w-full h-full pointer-events-none">
                      {[0, 1, 2, 3, 4].map((i) => (
                        <motion.line key={i} x1={`${20 + (i % 3) * 30}%`} y1={`${20 + Math.floor(i / 3) * 40}%`} x2={`${20 + ((i + 1) % 3) * 30}%`} y2={`${20 + Math.floor((i + 1) / 3) * 40}%`} stroke="rgba(0,0,0,0.08)" strokeWidth="1" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ delay: i * 0.1, duration: 0.4 }} />
                      ))}
                    </svg>
                    {prof.items.slice(0, 6).map((item, i) => (
                      <motion.div key={item.label} className="absolute flex flex-col items-center" style={{ left: `${15 + (i % 3) * 32}%`, top: `${15 + Math.floor(i / 3) * 42}%` }} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: i * 0.1, type: "spring", damping: 12 }}>
                        ...
                        <span className="text-[9px] text-ink mt-1 whitespace-nowrap bg-white/80 px-1 rounded shadow-sm">{item.label.split(" @ ")[0]}</span>
                      </motion.div>
                    ))}
                  </div>
                </motion.div>
